...
def load_feature_vectors(h5_path='/home/nick/Data/Datasets/hf_lob.h5',
                         n_features=5):
    """
    ...
    features = np.float32(features[:, mid_feature - n_features +
                                   1:mid_feature + 1, :])
...
if __name__ == '__main__':
    ...
    features, targets, day_train_split_idx, day_test_split_idx, stock_idx = load_feature_vectors(
        n_features=1)
    features = features.reshape(-1, 144)

    file = h5py.File(
        '/home/nick/Data/Workspace/Python/2018/lob_rnn/lob_utils/lob_raw.h5',
        'w')
    file.create_dataset('features', data=features, dtype='float32')
    file.create_dataset('targets', data=targets, dtype='int32')
    file.create_dataset('day_train_split_idx',
                        data=day_train_split_idx,
                        dtype='bool')
    file.create_dataset('day_test_split_idx',
                        data=day_test_split_idx,
                        dtype='bool')
